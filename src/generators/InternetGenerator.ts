 ...
 */
export default class InternetGenerator implements IGenerator<InternetConfig, string> {
  ...
  email(config?: InternetConfig): string {
    ...
    const domain = cfg.domain || rng.pick(['gmail.com', 'yahoo.com', 'hotmail.com', 'example.com']);
  ...
  validateConfig(config: InternetConfig): boolean {
    ...
    if (config.usernameLength !== undefined) {
      if (typeof config.usernameLength !== 'number' || config.usernameLength < 1) {
        return false;
      }
    }
